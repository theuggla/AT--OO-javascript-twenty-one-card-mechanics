...
router.opts('/users/:id/desiredtrips', passport.authenticate('jwt', { session: false }), mw.getAuthLevel, (req, res, next) => { req.user.authorized ? opts.updateResource(req, res, next) : opts.safeResource(req, res, next) })
router.get('/users/:id/desiredtrips', passport.authenticate('jwt', { session: false }), mw.getAuthLevel, user.info)
router.put('/users/:id/desiredtrips', passport.authenticate('jwt', { session: false }), mw.authorize, user.update)
router.del('/users/:id/desiredtrips', passport.authenticate('jwt', { session: false }), mw.authorize, user.delete)
